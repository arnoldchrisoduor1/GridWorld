...
import { ACTIONS, ACTION_SYMBOLS, CELL_TYPES } from "../../utils/constants.js";
...
const QValueHeatmap = ({
  ...
  const getCellQValues = useCallback(
    (row, col) => {
      if (!qTable) return {};
  ...
  const normalizeQValue = useCallback(
    (value, row, col) => {
  ...
  const getQValueColor = useCallback(
    (value, row, col) => {
  ...
  const getCellOverlay = useCallback(
    (row, col) => {
      ...
      const avgValue =
        Object.values(qValues).reduce((a, b) => a + b, 0) /
        Object.values(qValues).length;
      ...
      return (
        <div
          ...
          style={{
            ...
            opacity: localSettings.opacity,
            color:
              normalizeQValue(displayValue, row, col) > 0.5 ? "white" : "black",
          }}
        >
          {localSettings.showValues && (
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) =>
                  setLocalSettings((prev) => ({
                    ...prev,
                    showBestAction: e.target.checked,
                  }))
                }
                className="rounded"
              />
        ...
        <div className="grid grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <span className="ml-1 font-medium">
              {qValueStats.min.toFixed(2)}
            </span>
          </div>
          <div>
            <span className="text-gray-600">Max:</span>
            <span className="ml-1 font-medium">
              {qValueStats.max.toFixed(2)}
            </span>
          </div>
          <div>
            <span className="text-gray-600">Range:</span>
            <span className="ml-1 font-medium">
              {qValueStats.range.toFixed(2)}
            </span>
          </div>
          <div>
            <span className="text-gray-600">Mean:</span>
            <span className="ml-1 font-medium">
              {qValueStats.mean?.toFixed(2) || "N/A"}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {selectedAction && (
            <div className="text-sm text-blue-700">
              Showing: {selectedAction.toUpperCase()}{" "}
              {ACTION_SYMBOLS[selectedAction]}
            </div>
          )}
        </div>
      </div>
